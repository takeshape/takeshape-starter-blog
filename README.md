...
1. Set up and run the client application
    ...
    1. `npm install -g @takeshape/cli` - This will install the TakeShape CLI
    1. `npm install` - This will install all project specific dependencies
    1. `takeshape login` - Authenticate your local development environment with the TS service
    1. `takeshape link` - Follow the command prompts to set up your local environment to communicate with TS
...
1. Use the CLI to point your local development folder to the static site you just created.
    - `npx takeshape link` - Follow the command prompts
...
### TS Configuration
- `.takeshape-project.graphql` - Generated by `link`. Reference to the TS endpoint
- `.graphqlconfig` - Generated by `link`. Reference to the TS endpoint for local IDE GraphQL plugin config
- `.takeshaperc` -  Generated by `link`.  GraphQL schema file for local IDE GraphQL plugin config
